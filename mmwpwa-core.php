...
function mmwpwa_load_scripts()
{
    ...
    if ($mmwpwastatus == true) {
        ...
        } elseif ($mmwpwastyle == 'style4') {
            wp_enqueue_style('style4-css', plugins_url('scripts/style4.css', __FILE__), array(), '1.0.0', 'all');
            wp_enqueue_script('style4-js', plugins_url('scripts/style4.js', __FILE__), array('jquery'), '1.0.0', true);
        } else {
            // wait
        }
    }
